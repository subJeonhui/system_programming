...
{ 
	...
	for (i=0; i < TCOUNT; i++)  { 
		...
		count++; 
		/* Check the value of count and 11_signal waiting thread when condition is reached. Note that this occurs while mutex is locked. */
...
{ 
	...
	while (count < COUNT_LIMIT) { 
		...
		printf("watch_count(): thread %d Condition 11_signal received.\n", *my_id);
	} 
	pthread_mutex_unlock(&count_mutex); 
	pthread_exit(NULL); 
} 
